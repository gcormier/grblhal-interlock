...
Monitors any number of digital input or output ports. If **any** monitored port is in its configured *active* state when the spindle is commanded on (`M3`/`M4`), the spindle hardware is suppressed and a warning is emitted.

> The sender still receives `ok` — grblHAL has no hard-error path for `M3`/`M4` — but the spindle does **not** start.
...
Guards specific **output** ports from being written while the spindle is running. If an `M62`/`M63`/`M64`/`M65` targets a guarded port while the spindle is on, the write is suppressed and a warning is emitted.
...
Input and output `P`-spaces are **independent** (output `P0` and input `P0` are different pins), which is why each forward slot has its own direction setting. On this workspace's board these map as documented in [`IO.md`](../IO.md), e.g. output `P0` = Spindle Clamp, `P1` = Spindle Release.

> **Limitation:** ports *claimed* by another plugin are compacted out of the macro `Pxxx` numbering, so a claimed port cannot be monitored by macro number. The ATC use cases (drawbar sensor input + release output) are unclaimed macro ports, so this is not a problem in practice.
...
That needs three forward slots (two outputs + the drawbar input) and two inverse slots (guarding
the outputs). The drawbar sensor is an **input**, so the inverse interlock doesn't apply to it —
it's just an extra spindle-start check; the solenoid that drives the drawbar is already guarded by
...
When an interlock fires, a warning message is pushed to the sender's console:
...
```sh
git submodule add https://github.com/youruser/grblhal-interlock plugins/interlock
```
...
### 3. Register the init call

In your driver's `grbl/plugins_init.h`, add (before the odometer block, which must remain last so it can claim the remaining NVS):
...
lib_deps =
    file://grblhal-interlock
```

**Cache note:** PlatformIO caches `file://` libraries in `.pio/libdeps/`. After editing plugin source files you must clear the cache or your changes will be ignored:

```sh
rm -rf .pio/libdeps/<env-name>/grblhal-interlock
```
...
- Tested on STM32F4xx (LongBoard32); should work on any grblHAL driver with ioports and output readback
